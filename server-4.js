...
const LABEL_WIDTH = process.env.LABEL_WIDTH || "60mm";
const LABEL_HEIGHT = process.env.LABEL_HEIGHT || "20mm";
const LABEL_COPIES = Math.max(1, Number.parseInt(process.env.LABEL_COPIES || "3", 10) || 3);
  ...
  try {
    ...
    if (html) {
      ...
      try {
        ...
        // Chromium memakai 96 CSS pixel per inci, bukan resolusi head Argox.
        const MM_TO_CSS_PX = 96 / 25.4;
        const LABEL_WIDTH_PX = Math.ceil(60 * MM_TO_CSS_PX);
        const LABEL_HEIGHT_PX = Math.ceil(20 * MM_TO_CSS_PX);
